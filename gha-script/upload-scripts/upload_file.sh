...
if [[ $(echo "$token_request" | jq -r '.errorCode') == "null" ]]; then
    ...
    response=$(curl -X PUT -H "Authorization: bearer $token" -H "Content-Type: application/gzip" -T $1 "https://s3.us-east.cloud-object-storage.appdomain.cloud/ose-power-toolci-bucket-stag/$PACKAGE_NAME/$VERSION/$1")
