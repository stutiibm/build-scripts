...
resolve_docker_image() {
  ...
  # Extract the major UBI version integer (the digit(s) right after UBI/UBI:/UBI<space>)
  local major
  major=$(echo "$upper" | grep -oP 'UBI\s*[: ]?\s*\K[0-9]+' | head -1)
  ...
  if [ -z "$full" ]; then
    ...
    echo "⚠️  No minor version in TESTED_ON='$raw', defaulting to UBI ${full}"
  fi
