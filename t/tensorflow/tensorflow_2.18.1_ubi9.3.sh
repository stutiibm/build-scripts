...
wget https://raw.githubusercontent.com/stutiibm/build-scripts/refs/heads/sbm-re2-py-replica/t/tensorflow/tf_2.18.1_fix.patch
...
if ! (bazel test --config=opt -k  //tensorflow/tools/pip_package/...); then
    # Check if the failure is specifically due to "No test targets were found"
    if bazel test --config=opt -k  //tensorflow/tools/pip_package/... 2>&1 | grep -q "No test targets were found"; then
