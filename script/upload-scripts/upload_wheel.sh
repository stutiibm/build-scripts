...
token_request=$(curl -X POST https://iam.cloud.ibm.com/identity/token \
  ...
  -d "grant_type=urn%3Aibm%3Aparams%3Aoauth%3Agrant-type%3Aapikey&apikey=$gha_currency_service_id_api_key_dev")
    ...
    if [[ $(echo "$token_request" | jq -r '.errorCode') == "null" ]]; then
        ...
        response=$(curl -X PUT -H "Authorization: bearer $token" -H "Content-Type: application/octet-stream" -T $1 "https://s3.us.cloud-object-storage.appdomain.cloud/ose-power-artifacts-stag/$PACKAGE_NAME/$VERSION/$1")
    ...
    if [[ $(echo "$token_request" | jq -r '.errorCode') == "null" ]]; then
        token=$(echo "$token_request" | jq -r '.access_token')
        response=$(curl -X PUT -H "Authorization: bearer $token" -H "Content-Type: application/octet-stream" -T $1 "https://s3.us.cloud-object-storage.appdomain.cloud/ose-power-artifacts-pentesting/$PACKAGE_NAME/$VERSION/$1")
